...
01  TEST-RES    PIC 9 VALUE ZERO.
    ...
    88  TEST-RES-FAIL VALUES ARE 1 THRU 10.
...
TEST-VERTICALLY.
    *> if value found in the horizontal row, indicate it TEST-RES
    PERFORM VARYING TIDX FROM 1 BY 1 UNTIL TIDX > 9 OR TEST-RES-FAIL
...
TEST-HORIZONTALLY.
    *> if value found in the vertical row, indicate it TEST-RES
    PERFORM VARYING TIDY FROM 1 BY 1 UNTIL TIDY > 9 OR TEST-RES-FAIL
...
TEST-SQUARE.
    ...
    *> table indexes start from 1
    ...
    PERFORM VARYING TIDX FROM TIDX-START BY 1 UNTIL TIDX = TIDX-LIM OR TEST-RES-FAIL
        PERFORM VARYING TIDY FROM TIDY-START BY 1 UNTIL TIDY = TIDY-LIM OR TEST-RES-FAIL
